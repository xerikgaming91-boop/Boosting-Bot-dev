    ...
    <header className="border-b border-zinc-800 bg-zinc-950/80 backdrop-blur supports-[backdrop-filter]:bg-zinc-950/60">
      <div className="max-w-6xl mx-auto px-4 h-14 flex items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className="size-7 rounded bg-indigo-600/80 grid place-items-center font-bold text-xs text-white">BB</div>
          <span className="text-zinc-100 font-semibold tracking-wide">Boosting Bot</span>
        </div>
        ...
        <nav className="flex items-center gap-1">
          ...
          {!loading && (user?.isRaidlead || user?.isAdmin || user?.isOwner) && <Tab to="/users" label="Users" />}
        </nav>
    ...
    <NavLink
      ...
      className={({ isActive }) =>
        [
          ...
          isActive ? "bg-zinc-800 text-zinc-100" : "text-zinc-300 hover:text-white hover:bg-zinc-800/60",
        ].join(" ")
      }
    >
      {label}
  ...
  return (
    <div className="flex items-center gap-3">
      <div className="flex items-center gap-2">
        {user.avatarUrl ? (
          <img src={user.avatarUrl} alt={name} className="size-7 rounded-full ring-1 ring-zinc-700 object-cover" referrerPolicy="no-referrer" />
        ) : (
          <div className="size-7 rounded-full bg-zinc-800 grid place-items-center text-xs text-zinc-300">{name?.[0]?.toUpperCase() || "U"}</div>
        )}
      ...
      <a href="/api/auth/logout" className="inline-flex items-center px-2.5 py-1.5 rounded-md bg-zinc-800 text-zinc-200 text-sm hover:bg-zinc-700 transition" title="Logout">
        Logout
      </a>
    </div>
  );
...
function RoleBadge({ label, color }) {
  return <span className={["inline-flex items-center px-2 py-0.5 rounded-md text-[10px] leading-4 font-medium text-white", "bg-gradient-to-r", color].join(" ")}>{label}</span>;
}
